...
class MessageFileHandler(FileSystemEventHandler):
    ...
    def _safe_process_file(self, file_path, event_type):
        ...
        # 添加防止短消息循环的检查
        if not hasattr(self, 'file_process_count'):
            self.file_process_count = {}
        if not hasattr(self, 'file_process_reset_time'):
            self.file_process_reset_time = {}
        if not hasattr(self, 'file_content_cache'):
        ...
        try:
            ...
            # 构建文件标识（大小+修改时间）
            file_signature = f"{file_size}_{file_mtime}"
            
            # 如果文件内容没有变化，跳过处理
            if file_path in self.file_content_cache:
                if self.file_content_cache[file_path] == file_signature:
                    logger.debug(f"文件 {file_path} 内容未变化，跳过处理")
                    return
            
            # 更新文件内容缓存
            self.file_content_cache[file_path] = file_signature
            
        except Exception as e:
            logger.warning(f"检查文件变化时出错: {e}")
        
        # 每5分钟重置文件处理计数
        if file_path in self.file_process_reset_time:
            if current_time - self.file_process_reset_time[file_path] > 300:  # 5分钟
                self.file_process_count[file_path] = 0
                self.file_process_reset_time[file_path] = current_time
        else:
            self.file_process_count[file_path] = 0
            self.file_process_reset_time[file_path] = current_time
        
        # 检查文件处理频率，防止短消息导致的无限循环
        self.file_process_count[file_path] = self.file_process_count.get(file_path, 0) + 1
        if self.file_process_count[file_path] > 10:  # 5分钟内最多处理10次
            logger.warning(f"文件 {file_path} 处理频率过高 ({self.file_process_count[file_path]}次)，跳过此次处理")
            return
    ...
    def process_file(self, file_path):
        try:
            ...
            if not content:
                ...
                # 即使内容为空也记录消息ID，避免重复处理
                if message_id:
                    self.processed_message_ids.add(message_id)
                    self._save_processed_ids()
                return
            
            # 防止短消息导致循环处理 - 添加内容长度检查
            content_stripped = content.strip()
            if len(content_stripped) < 10:
                # 对于非常短的消息（1-3字符），直接跳过并记录
                if len(content_stripped) <= 3:
                    logger.info(f"消息内容过短 ({len(content_stripped)} 字符)，跳过处理: {content_stripped}")
                    # 记录短消息的处理状态，避免重复
                    if message_id:
                        self.processed_message_ids.add(message_id)
                        self._save_processed_ids()
                    ...
                    # 对于稍长的短消息，检查是否包含交易相关关键词
                    trading_keywords = ['做多', '做空', '买入', '卖出', '止损', '止盈', 'BTC', 'ETH', '比特币', '以太坊']
                    has_trading_content = any(keyword in content_stripped for keyword in trading_keywords)
                    
                    if not has_trading_content:
                        logger.info(f"短消息无交易内容 ({len(content_stripped)} 字符)，跳过处理: {content_stripped}")
                        # 记录短消息的处理状态，避免重复
                        if message_id:
                            self.processed_message_ids.add(message_id)
                            self._save_processed_ids()
                        ...
                        logger.info(f"短消息包含交易内容，继续处理: {content_stripped}")
    ...
    def _update_complete_excel(self, output_dir):
        ...
        try:
            ...
            # 在保存前应用智能合并筛选逻辑
            df = self.merge_similar_messages(df)
    ...
    def merge_similar_messages(self, df):
        ...
        # 合并参数配置
        config = {
            "time_window_hours": 4,           # 时间窗口（4小时）
            "entry_tolerance": 0.0,           # 入场点位容差（0% - 完全相同）
                        ...
                        if is_update:
                            ...
                            if entry1 and next_entry1 and not pd.isna(entry1) and not pd.isna(next_entry1):
                                try:
                                    ...
                                    # 完全相同的入场点位（容差为0）
                                    should_merge = entry1 == next_entry1
...
"""
    ...
    def should_analyze_message(self, msg: Dict, channel_name: str = None) -> bool:
        ...
        try:
            ...
            # 检查消息中是否包含BTC或ETH相关内容
            btc_keywords = ['BTC', '比特币', '大饼']
            eth_keywords = ['ETH', '以太', '以太坊', '二饼']
            
            has_btc = any(keyword in content for keyword in btc_keywords)
            has_eth = any(keyword in content for keyword in eth_keywords)
            
            # 如果消息不包含BTC或ETH相关内容，直接分析
            ...
            try:
                ...
                if not hasattr(self, 'price_monitor'):
                    self.price_monitor = BinanceRestPriceMonitor(polling_interval=5)
                    self.price_monitor.initialize()
                
                # 检查BTC价格
                if has_btc:
                    btc_price = self.price_monitor.get_current_price('BTCUSDT')
                    if btc_price is not None and btc_price < 80000:
                        logger.info(f"跳过BTC相关消息：当前价格 {btc_price} 低于阈值 80000")
                        return False
                
                # 检查ETH价格
                if has_eth:
                    eth_price = self.price_monitor.get_current_price('ETHUSDT')
                    if eth_price is not None and eth_price < 2500:
                        logger.info(f"跳过ETH相关消息：当前价格 {eth_price} 低于阈值 2500")
                        return False
                
                logger.info("价格检查通过，继续分析消息")
                return True
                
            except Exception as e:
                logger.warning(f"价格检查失败，继续分析消息: {str(e)}")
                return True
                
        except Exception as e:
            logger.error(f"should_analyze_message 方法出错: {str(e)}")
            return True
    ...
    def analyze_message(self, content: str, channel_name: str = None, retry_count: int = 3) -> Optional[Dict]:
        ...
        try:
            ...
            if not self.should_analyze_message(msg_dict, channel_name):
                logger.info("消息因价格检查未通过而被跳过")
                return None
    ...
    def _call_api_with_retry(self, messages, content_to_analyze, original, translated, extracted_info, channel_name, retry_count=3):
        ...
        if api_result is None:
            ...
            empty_result = self._create_empty_result(original, translated, channel_name, content_to_analyze)
            # 确保空结果也被记录为已处理
            return empty_result
    ...
    def start_monitoring(self, path: str):
        ...
        try:
            ...
            if not hasattr(self, 'last_restart_time'):
                self.last_restart_time = 0
            
            current_time = time.time()
            
            # 如果距离上次重启小于1分钟，增加重启计数
            ...
            if self.restart_count < 3:  # 最多重试3次
                wait_time = min(10 * self.restart_count, 60)  # 递增等待时间，最多60秒
                logger.info(f"第{self.restart_count}次重新启动监控，{wait_time}秒后尝试...")
                time.sleep(wait_time)
                self.start_monitoring(path)
            else:
                logger.critical(f"监控重启失败次数过多({self.restart_count})，停止重试")
                logger.critical("请检查系统配置或手动重启程序")
                # 不要重置计数器，让它保持高值以防止继续重试
...
def apply_message_filtering(df):
    """独立的消息筛选函数，实现与FileAnalyzer.merge_similar_messages相同的逻辑"""
    if df is None or df.empty:
        logger.info("输入数据为空，跳过筛选逻辑")
        return df
    
    logger.info("\n=== 独立数据合并过程 ===")
    logger.info(f"初始数据行数: {len(df)}")
    
    # 如果只有一条记录，也执行筛选逻辑（用于完整性检查）
    if len(df) == 1:
        logger.info("只有一条记录，但仍执行筛选逻辑进行完整性检查")
    ...
    try:
        # 检查必要的列是否存在
        required_columns = ['channel', 'analysis.交易币种', 'analysis.方向']
        missing_columns = [col for col in required_columns if col not in df.columns]
        
        if missing_columns:
            logger.warning(f"缺少必要的列进行分组: {missing_columns}")
            logger.info("跳过分组筛选，直接返回原数据")
            return df
    ...
    try:
        ...
        for (channel, currency, direction), group in grouped:
            ...
            for i, record in enumerate(group_records):
                # 创建唯一标识符（如果没有message_id则使用索引）
                record_id = record.get('message_id') or f"record_{i}"
                ...
                for j in range(i+1, len(group_records)):
                    next_record = group_records[j]
                    next_record_id = next_record.get('message_id') or f"record_{j}"
                    
                    if next_record_id in processed_ids:
                        continue
                    ...
                    if should_merge:
                        processed_ids.add(next_record_id)
                        merged_with_any = True
        ...
        logger.error(f"独立合并消息时出错: {str(e)}")
        return df
    ...
    try:
        ...
        if os.path.exists(output_path):
            try:
                ...
                # 合并数据
                # 过滤掉所有空列，避免FutureWarning
                df_new_filtered = df_new.dropna(axis=1, how='all')
                combined_df = pd.concat([existing_df, df_new_filtered], ignore_index=True)
                ...
                if os.path.exists(excel_path):
                    try:
                        ...
                        # 应用智能合并筛选逻辑
                        # 注意：这里我们需要创建一个临时的分析器实例来调用merge_similar_messages
                        # 由于这个函数是独立的，我们需要在此处直接实现简化的筛选逻辑
                        filtered_df = apply_message_filtering(combined_df)
                        filtered_df.to_excel(writer, sheet_name='所有记录', index=False)
                        writer.close()
                        logger.info(f"Excel数据已更新: {excel_path}")
                    except Exception as e:
        ...
        try:
            writer = pd.ExcelWriter(excel_path, engine='openpyxl')
            # 应用智能合并筛选逻辑
            filtered_df = apply_message_filtering(df_new)
            filtered_df.to_excel(writer, sheet_name='所有记录', index=False)
            writer.close()
